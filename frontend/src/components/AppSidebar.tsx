...
export function AppSidebar() {
  ...
  const notebookId = pathname.includes("/notebook/")
    ? pathname.split("/notebook/")[1]?.split("/")[0]
    : null;
  ...
  const handleDeleteFile = async (filename: string) => {
    ...
    try {
      const res = await secureFetch(
        `${API_BASE}/api/notebooks/${notebookId}/files/${filename}`,
        {
          method: "DELETE",
        },
      );
  ...
  return (
    <aside
      className={`h-screen bg-[#0B0C15] border-white/10 flex flex-col flex-shrink-0 transition-all duration-300 z-50 relative ${isCollapsed ? "w-0 border-r-0" : "w-72 border-r"}`}
    >
      {/* Toggle Button */}
      <button
        onClick={toggleSidebar}
        className="absolute top-6 -right-10 w-10 h-10 bg-[#0B0C15] border border-white/10 border-l-0 rounded-r-xl flex items-center justify-center text-slate-500 hover:text-indigo-400 z-50 transition-colors shadow-lg"
      ...
      {/* Inner Wrap */}
      <div
        className={`flex flex-col h-full w-72 overflow-hidden transition-opacity duration-300 ${isCollapsed ? "opacity-0 pointer-events-none" : "opacity-100"}`}
      >
        {/* --- HEADER --- */}
            ...
            <div className="flex items-center gap-2 animate-in fade-in slide-in-from-left-2 duration-300">
              <Link href="/dashboard">
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-8 w-8 hover:bg-white/10 text-slate-400 shrink-0"
                >
            ...
            <div className="animate-in fade-in slide-in-from-right-2 duration-300">
              ...
              >
                <img
                  src="/icon.png"
                  alt="Cortex Icon"
                  className="w-8 h-8 rounded-lg shadow-lg shadow-indigo-500/20 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300"
                />
                <span className="font-bold text-2xl tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400 group-hover:from-indigo-400 group-hover:to-purple-400 transition-all font-[family-name:var(--font-orbitron)]">
                  CORTEX
                </span>
              </Link>
              ...
              >
                <button className="w-full flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 py-2 rounded-lg text-sm font-medium transition-all group outline-none focus-visible:ring-2 focus-visible:ring-indigo-500">
